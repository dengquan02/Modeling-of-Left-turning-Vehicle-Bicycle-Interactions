[input]=xlsread('E:\Prediction_NV\Data_preprocessing\数据清洗整理\原始数据\昌吉东路墨玉路机动车.xlsx',3);
%datdawash
%寻找起终点在特定区域的轨迹
%去除问题数据
input(:,1)=[];
...
%     if input(i,13)==1&&-10<input(i,2)&&input(i,2)<60&&10<input(i,3)&&input(i,3)<60%判断起点是否在指定区域
%         AAA=AAA+1;
%         sp=i;
...
%                end
%            end
%         if -10<input(sp,2)&&input(sp,2)<60&&10<input(sp,3)&&input(sp,3)<60%判断终点是否在指定区域
%            AA=AA+1;
...
s = xlswrite('data.xlsx',result,11);  
