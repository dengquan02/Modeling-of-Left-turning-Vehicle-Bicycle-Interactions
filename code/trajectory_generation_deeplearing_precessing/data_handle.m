...
for i=1:size(E_straight,1)%给每个直行轨迹点都弄好特征
    ...
    for i1=1:size(NVinterract,1)%找一个轨迹点的所有交互非机动车，并存放在E2中
        if NVinterract(i1,1)==E_straight(i,1)&&NVinterract(i1,2)~=E_straight(i,2)&&NVinterract(i1,3)~=E_straight(i,3)%判断是否存在相同时间的轨迹点（包括原轨迹点）
           nu=nu+1;%交互对象计数
...
for i=1:size(E_straight,1)%给每个轨迹点都弄好特征
    ...
    for i1=1:size(Vinterract,1)%找一个轨迹点的特征
        if Vinterract(i1,1)==E_straight(i,1)&&Vinterract(i1,2)~=E_straight(i,2)&&Vinterract(i1,3)~=E_straight(i,3)%判断是否存在相同时间的轨迹点
           nu_V=nu_V+1;%交互对象计数
