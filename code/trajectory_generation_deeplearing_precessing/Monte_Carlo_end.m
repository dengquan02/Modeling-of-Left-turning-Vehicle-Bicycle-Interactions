function [aim_danger,Nu_part] = Monte_Carlo_end(intrusion_cood_E,char_end,M,frequency_NV,long,short,index_end)
% tic
%通过蒙特卡洛模拟求出离终点的危险度聚集程度，并用概率点进行危险度计算
...
% char_end = 0.8;%标定的参数,0-1之间
% frequency_NV = 10^6;%动态交互的实验次数,根据此次数求终点危险度的实验次数，与实验体积有关
...
RandData(:,3) = RandData(:,3)*M;%Z坐标，危险度

%筛选
ell = zeros(size(RandData,1),1);
for i = 1:size(RandData,1)%计算是否符合要求
    ell(i,1) = (RandData(i,3)<=(char_end*(index_end.^(-RandData(i,1)))));%只筛选出z坐标低于函数的值（利用y值计算）就行，x坐标不用管       
end
