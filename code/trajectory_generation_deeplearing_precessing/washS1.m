[input]=xlsread('E:\Prediction_NV\Data_preprocessing\数据清洗整理\原始数据\昌吉东路墨玉路非机动车.xlsx',3);
%datdawash
%寻找起终点在特定区域的轨迹
%该脚本没有剔除孤立点步骤，在N2中有
input(:,1)=[];
...
%     if input(i,13)==1&&25<input(i,2)&&input(i,2)<40&&-5<input(i,3)&&input(i,3)<20%判断起点是否在指定区域
%         AAA=AAA+1;
%         sp=i;
...
%                end
%            end
%         if 25<input(sp,2)&&input(sp,2)<38&&30<input(sp,3)&&input(sp,3)<55%判断终点是否在指定区域
%            AA=AA+1;
