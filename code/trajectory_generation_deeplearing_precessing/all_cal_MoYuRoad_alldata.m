%% 该代码仅适用于  result标定参数_完善风险场标定_平均数约束_全部数据1237(0112_2)
perception_time = 20;
global intrusion_cood_E
intrusion_cood_E = csvread('intrusion_cood_EXXRoad.csv');%读取基准   运行墨玉路时是：intrusion_cood_E = csvread('intrusion_cood_E.csv');%读取基准
benchmark_E = csvread('benchmark_EXXRoad.csv');%读取基准信息      运行墨玉路时是：benchmark_E = csvread('benchmark_E.csv');%读取基准信息
...
for ID = 1:input(size(input,1),1)%计算每条轨迹   %for ID = 1:size(index_test,2)%计算每条轨迹
    index = ID;%轨迹编号  这个插回来
    output = [];
    extract_trajectory = input((input(:,1)==index),:);%提取轨迹编号为index的轨
%     extract_trajectory = input(ID:ID+perception_time,:);
    index_in = randperm(extract_trajectory(size(extract_trajectory,1),2)-perception_time-1,1);%随机选出轨迹的一个时刻来进行预测   extract_trajectory(1,2);%
    global extract_one
...
%        Parameter = Parameter_cal;%标定的参数
      Parameter = [1 0.8 1 5 0.2 0.12 1 5 0.2 0.03];%手动填的参数(10个)  %Parameter = [2 0.01 1 1 0.05 2 0.2 2];%手动填的参数
       MyGA;%用遗传算法求解
%        MyGa_Moeth3;%方法3求解
...
%     test_result_precessing = [[floor(ID/(perception_time+1)) index index_in OPT]];
    ...
    disp(['已完成',num2str(ID/input(size(input,1),1)*100),'%'])   %disp(['已完成',num2str(ID/size(input,1)*100),'%'])
end
toc
...
for i =1:size(test_result_precessing,1)
    ...
    [output,output1,output2] = Excur_predict_2(test_result_precessing(i,4:size(test_result_precessing,2)),Truth_no(1,:),Truth_no,perception_time);%%output是目标值，output1是输出的预测点；output2是输出的风险值；
...
% %         scatter(Truth_no(j+1,4),Truth_no(j+1,5),35,'+','black')
%         hold on
%         scatter(output1(j+1,1),output1(j+1,2),35,'*','black')  %o v *
% %         pause(0.5)
%     end
%     axis([-10 10 15 35]);
...
% ILP(find(isnan(ILP)),:)=[];
%% 保存
save('result仙霞路全部数据轨迹预测计算(0531_1)')
