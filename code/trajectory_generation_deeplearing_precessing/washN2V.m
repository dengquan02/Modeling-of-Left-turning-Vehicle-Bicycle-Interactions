[input]=xlsread('E:\Prediction_NV\Data_preprocessing\数据清洗整理\原始数据\昌吉东路墨玉路机动车.xlsx',2);
%datdawash
%寻找起终点在特定区域的轨迹
%去除问题数据
input(:,1)=[];
...
for i=1:size(input,1)%剔除错误点
    if 20<input(i,2)||input(i,2)<-10||60<input(i,3)||input(i,3)<-10%剔除条件，孤立点
...
%     if input(i,13)==1&&-10<input(i,2)&&input(i,2)<25&&-10<input(i,3)&&input(i,3)<60%判断起点是否在指定区域
%         AAA=AAA+1;
%         sp=i;
...
s = xlswrite('data.xlsx',result,10);  
