...
[E1]=xlsread('M:\New_try\分类完成数据（T=0.12s）.xlsx',1);%东进口直行非机动车
[E1V]=xlsread('M:\New_try\分类完成数据（T=0.12s）.xlsx',2);%东进口直行机动车
[W1V]=xlsread('M:\New_try\分类完成数据（T=0.12s）.xlsx',3);%西进口左转机动车
id_amount=max(E1(:,12))
...
% scatter(E1(:,16),E1(:,17),'.','r');%做图
% scatter(E1V(:,16),E1V(:,17),'.','r');%做图
% scatter(W1V(:,16),W1V(:,17),'.','r');%做图
...
testdividing(x_test,'M:\New_try\trajectory_generation_deeplearingModel\data_test');  %地址是保存测试样本的地址
