[input]=xlsread('E:\Prediction_NV\Data_preprocessing\数据清洗整理\原始数据\昌吉东路墨玉路非机动车.xlsx',4);
%datdawash
%寻找起终点在特定区域的轨迹
%去除问题数据
input(:,1)=[];
...
for i=1:size(input,1)%剔除错误点
    if 50<input(i,2)||input(i,2)<-10||100<input(i,3)||input(i,3)<-100%剔除条件，孤立点
...
%     if input(i,13)==1&&26<input(i,2)&&input(i,2)<40&&-5<input(i,3)&&input(i,3)<20%判断起点是否在指定区域
%         AAA=AAA+1;
%         sp=i;
...
%                end
%            end
%         if 26<input(sp,2)&&input(sp,2)<38&&30<input(sp,3)&&input(sp,3)<50%判断终点是否在指定区域
%            AA=AA+1;
