...
tic;
%% 读取数据
type = 2; %车辆类型，1是自行车，2是电动车，3是机动车，4是外卖骑手
filename = 'D:\File\Received File\交科赛大三\资料（交科0106）\按照外轮廓四个世界坐标转换后轨迹-初始轨迹\重新编号\4-桂林路吴中路东侧-转换后轨迹数据（三分类）（平均数填充缺失值）(已处理).xlsx';
%将每个轨迹点对应的机动车和非机动车坐标放到同一行上,14列
[E1]=xlsread('D:\File\Received File\交科赛大三\资料（交科0106）\按照外轮廓四个世界坐标转换后轨迹-初始轨迹\重新编号\4-桂林路吴中路东侧-转换后轨迹数据（三分类）（平均数填充缺失值）.xlsx',type);%自行车
%% 输入参数
%轨迹编号 时间 x y
Fs = 25;  %采样率
...
for i = 1:E1(end,12)
    ...
    tempVx = [E1(ID,4)];
    tempVy = [E1(ID,5)];
    tempVx(2:end-1) = (x(ID(3:end))-x(ID(1:end-2)))./(t(ID(3:end))-t(ID(1:end-2)));
    tempVy(2:end-1) = (y(ID(3:end))-y(ID(1:end-2)))./(t(ID(3:end))-t(ID(1:end-2)));
    Vx(ID) = tempVx;
    Vy(ID) = tempVy;
    %计算加速度
    tempAx = [E1(ID,6)];
    tempAy = [E1(ID,7)];
    tempAx(2:end-1) = (x(ID(3:end))-2*x(ID(2:end-1))+x(ID(1:end-2)))./(t(ID(2:end-1))-t(ID(1:end-2))).^2;
    tempAy(2:end-1) = (y(ID(3:end))-2*y(ID(2:end-1))+y(ID(1:end-2)))./(t(ID(2:end-1))-t(ID(1:end-2))).^2;
    Ax(ID) = tempAx;
    Ay(ID) = tempAy;
    A(ID(2:end)) = (V(ID(2:end))-V(ID(1:end-1)))./0.04;
end
V = (Vx.^2+Vy.^2).^0.5.*3.6;
...
for i = 1:E1(end,12)
    ...
    %⑥重新计算位置和加速度
    for j = 2:length(ID)
        x(ID(j)) = x(ID(j-1)) + (Vx(ID(j-1)))*0.04;
        y(ID(j)) = y(ID(j-1)) + (Vy(ID(j-1)))*0.04;
        Ax(ID(j)) = (Vx(ID(j)) - Vx(ID(j-1)))/0.02;
        Ay(ID(j)) = (Vy(ID(j)) - Vy(ID(j-1)))/0.02;
    end
    A(ID(2:end)) = (V(ID(2:end))-V(ID(1:end-1)))./0.04;
end
V = (Vx.^2+Vy.^2).^0.5.*3.6;
...
xlswrite(filename,[title;num2cell(data)],sheetname);
