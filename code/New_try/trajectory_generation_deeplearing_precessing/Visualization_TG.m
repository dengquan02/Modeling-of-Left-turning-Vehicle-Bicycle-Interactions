%% 读取该目录下的所有txt文件
path = 'D:\File\Received File\交科赛大三\非机动车行为建模代码（TOPS）\非机动车行为建模代码（TOPS）\New_try\trajectory_generation_deeplearingModel\result_test\';%python的预测结果地址
namelist = dir('D:\File\Received File\交科赛大三\非机动车行为建模代码（TOPS）\非机动车行为建模代码（TOPS）\New_try\trajectory_generation_deeplearingModel\result_test\*.txt');%读取所有txt文件名  %python的预测结果地址
