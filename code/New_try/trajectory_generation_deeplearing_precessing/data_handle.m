function [output_E] = data_handle(E_straight,NVinterract,Vinterract,Num_No,Num_V,index_zeros)
...
all_NVinterract = zeros(size(E_straight,1),Num_No*11);%All of interactive NV
chosed_NVinterract = zeros(size(E_straight,1),Num_No*11);%Selected of interactive NV

for i=1:size(E_straight,1)%给每个直行轨迹点都弄好特征
    nu=0;
    for i1=1:size(NVinterract,1)%找一个轨迹点的所有交互非机动车，并存放在E2中
        if abs(NVinterract(i1,1)-E_straight(i,1))<=0.04&&NVinterract(i1,2)~=E_straight(i,2)&&NVinterract(i1,3)~=E_straight(i,3)%判断是否存在相同时间的轨迹点（包括原轨迹点）
           nu=nu+1;%交互对象计数
    ...
    Nu_NVinterract(i,1)=nu;%number of interract NV  
    d=zeros(2,max(nu,Num_No));%存放交互两者之间的距离，选取距离最短的Num个（不为0）
    for i2=1:max(nu,Num_No)
        if  all_NVinterract(i,(i2*11)-10)~=0%若交互对象不为0的话
            d(1,i2)=i2;%第一行放他们的编号
            d(2,i2)=((E_straight(i,2)- all_NVinterract(i,(i2*11)-10))^2+((E_straight(i,3)- all_NVinterract(i,(i2*11)-9))^2))^0.5;%第二行放预测主体和交互客体的距离
       end
    end
...
for i=1:size(E_straight,1)%给每个轨迹点都弄好特征
    ...
    for i1=1:size(Vinterract,1)%找一个轨迹点的特征
        if abs(Vinterract(i1,1)-E_straight(i,1))<=0.04&&Vinterract(i1,2)~=E_straight(i,2)&&Vinterract(i1,3)~=E_straight(i,3)%判断是否存在相同时间的轨迹点
           nu_V=nu_V+1;%交互对象计数
    ...
    Nu_Vinterract(i,1)=nu_V;   
    d2=zeros(2,max(nu_V,Num_V));%存放交互两者之间的距离，选取距离最短的Num个（不为0）
    for i2=1:max(nu_V,Num_V)
        if  all_Vinterract(i,(i2*11)-10)~=0%若交互对象不为0的话
            d2(1,i2)=i2;%第一行放他们的编号
            d2(2,i2)=((E_straight(i,2)- all_Vinterract(i,(i2*11)-10))^2+(E_straight(i,3)- all_Vinterract(i,(i2*11)-9))^2)^0.5;%第二行放他们的距离
       end
    end
