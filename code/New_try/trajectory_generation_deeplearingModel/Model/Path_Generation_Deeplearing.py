#该脚本的作用是利用已知的起终点进行轨迹生成，配合matlab端完成轨迹预测
# 以下开始
...
##若文件夹存在，则清空文件夹,若文件夹不存在，则创造
#filepath = r"E:\Prediction_NV\New_try\Data_processing\result_test"
...
model = load_model('trajectory_generation（1124_1）.h5')   #读取模型“cnnlstm（1223）.h5”   model = load_model('trajectory_generation（0926_2）.h5')   #读取模型“cnnlstm（1223）.h5”
model.summary()  ##打印出模型概况，它实际调用的是keras.utils.print_summary
n = 1  #计算计数
dir = r"D:\File\Received File\交科赛大三\非机动车行为建模代码（TOPS）\非机动车行为建模代码（TOPS）\New_try\Data_processing\data_test"  #r表示不转义，即字符串里的\就是\，而不转义为制表符   dir表示这个路径
...
for filename in filenames:  #路径下的文件有215+1个吗？
    x_input = np.loadtxt(open(r"D:\File\Received File\交科赛大三\非机动车行为建模代码（TOPS）\非机动车行为建模代码（TOPS）\New_try\Data_processing\data_test\{0}".format(filename), "rb"), #
                         delimiter=",", skiprows=0)  #大括号是字典字符，delimiter表示分割符  skiprows表示跳过头行  format格式化字符串的函数
    test_x = x_input.reshape(1, (n_input*n_feature))  #后面的模型需要（1,28）的形状，(行，列)
    y_pred = []  #定义list值
    PRED_SIZE = 17 #  生成只能是3/5/9/17/33步，每次加密   试试看各个步长的准确率  先试试17个步长的
    ...
    for level in range(0, 4):  #17个步长就是4次，33个步长就是5次    2+（1+2+4+8+16）
        output_j = based_x[0, :]  # 赋予初始起点
        for j in range(0, based_x.shape[0]-1, PRED_BATCH):  #range(start, stop, step)  ##对一个轨迹预测  PRED_SIZE=25 PRED_BATCH=1
            input_point = based_x[j:j+2, :].reshape(1, (n_input*n_feature)) #将输入点的计算位置换算成一行，以便放入预测中
            y_batch = model.predict(input_point)  #模型输入的维度是（1,28）输出=（1,14） [:,j]表示选定其中某列进行预测输入
            y_batch = y_batch[0, :]  #把二维数组变成1维
            ## 将预测到的相对值整合为绝对坐标
            #print(input_point[0, 0])
            y_batch[0] = input_point[0, 0] + y_batch[0] * (input_point[0, n_feature] - input_point[0, 0])  #n_feature是特征数
    ...
    csvfile = open(r'D:\File\Received File\交科赛大三\非机动车行为建模代码（TOPS）\非机动车行为建模代码（TOPS）\New_try\Data_processing\result_test\{0}'.format(filename), 'w', newline="") #打开文件，写入   newline 是分割形式，防止每行最后有空格
